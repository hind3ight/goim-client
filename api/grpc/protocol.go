...
func (p *Proto) WriteWebsocket(ws *websocket.Conn) (err error) {
	...
	binary.BigEndian.PutInt32(buf[_packOffset:], int32(packLen))          // 包长4字节
	binary.BigEndian.PutInt16(buf[_headerOffset:], int16(_rawHeaderSize)) // 包头长2字节
	binary.BigEndian.PutInt16(buf[_verOffset:], int16(p.Ver))             // 版本2字节
	binary.BigEndian.PutInt32(buf[_opOffset:], p.Op)                      // 操作4字节
	binary.BigEndian.PutInt32(buf[_seqOffset:], p.Seq)                    // 序列号4字节
	if p.Body != nil {
		err = ws.WriteBody(p.Body) // 消息
	}
	return
}
